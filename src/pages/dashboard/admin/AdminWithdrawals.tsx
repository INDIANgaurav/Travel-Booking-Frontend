...
export default function AdminWithdrawals() {
  ...
  const fetchRequests = async () => {
    setLoading(true);
    try {
      const url = filter === 'ALL' ? '/api/wallet/admin/withdrawals' : `/api/wallet/admin/withdrawals?status=${filter}`;
  ...
  const confirmSubmitAction = async () => {
    ...
    try {
      await api.put(`/api/wallet/admin/withdrawals/${id}`, { status: action === 'approve' ? 'APPROVED' : 'REJECTED' });
  ...
  const filteredRequests = requests.filter(req => 
    ...
    req.bankDetails?.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  return (
    <div className="p-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          <h1 className="text-2xl font-black text-[#0c1a40]">Withdrawal Requests</h1>
          <p className="text-gray-500 font-bold text-sm">Verify and approve agent withdrawal requests</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="p-4 border-b border-gray-200 flex flex-col sm:flex-row gap-4 justify-between items-center bg-gray-50">
          ...
          <div className="flex items-center gap-4 w-full sm:w-auto">
            ...
            <RefreshButton onClick={fetchRequests} loading={loading} count={filteredRequests.length} />
          </div>
        </div>
              ...
              {loading ? (
                ...
                filteredRequests.map((req) => (
                  <tr key={req._id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div className="text-xs text-gray-700 whitespace-pre-wrap">{req.bankDetails || 'N/A'}</div>
                    </td>
                    <td className="px-6 py-4">
