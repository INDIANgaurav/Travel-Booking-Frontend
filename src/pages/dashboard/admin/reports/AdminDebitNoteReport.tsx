...
export default function AdminDebitNoteReport() {
  ...
  const headers = [
    ...
    'Debit By', 'Debit To', 'Debit Amount', 'Remarks', 'Company', 'Action'
  ];
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await reportsApi.getDebitNotes({
        fromDate,
        toDate,
        limit: records
      });
  ...
  const handleDownload = () => {
    ...
    exportToCSV(data, 'Debit_Notes_Report');
  ...
  const renderRow = (row: any) => (
    <tr key={row.id} className="hover:bg-slate-50/50 transition-colors group">
      ...
      <td className="px-6 py-4 align-middle align-middle">{row.debitBy}</td>
      <td className="px-6 py-4 align-middle align-middle">{row.debitTo}</td>
      <td className="px-6 py-4 align-middle font-bold text-red-600">₹ {row.amount}</td>
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300 w-full overflow-hidden pb-20">
      <ReportHeader onRefresh={fetchData} 
        ...
        description="Track all manual and automated debit transactions across agencies"
      ...
      {loading ? (
        ...
        <ReportTable 
          ...
          data={data}
          renderRow={renderRow}
          emptyMessage="No debit notes found for this period"
        />
      )}
    </div>
  );
